...
namespace kcg {
	...
	struct EdgePointWithGradient {
		cv::Point2f pt;        // 边缘点坐标
		float grad_x;        // x方向梯度
		float grad_y;        // y方向梯度

	};
	struct smallEdgePointWithGradient {
		cv::Point pt;        // 边缘点坐标
		float grad_x;        // x方向梯度
		float grad_y;        // y方向梯度
	...
	typedef struct Template_S
	{
		...
		std::vector<smallEdgePointWithGradient>small_edge_features;
		std::vector<EdgePointWithGradient> edge_features;
	}Template;
	...
	struct MatchResult {
		double score = -1.0f;
		cv::Point2f  match_pos;
		float match_angle = 0.0f;
	};
	typedef enum PyramidLevel_E
	{
		...
		PyramidLevel_6 = 6,
		PyramidLevel_7 = 7,
		//PyramidLevel_TabooUse = 42,
		//PyramidLevel_TabooUse = 16,
		//PyramidLevel_TabooUse = 6,
		PyramidLevel_TabooUse = 6,
	}PyramidLevel;
	...
	public:
		...
		*/
		void MakingTemplates(Mat model, AngleRange angle_range, ScaleRange scale_range,
			int num_features, int binarize_thresh,float weak_thresh = 30.0f, float strong_thresh = 60.0f,
			Mat mask = Mat());
		void MakingTemplates1(Mat model, AngleRange angle_range, ScaleRange scale_range,
			int num_features, float weak_thresh = 30.0f, float strong_thresh = 60.0f,
			Mat mask = Mat());
		void MakingTemplates_fast(Mat model, AngleRange angle_range, ScaleRange scale_range,
			int num_features, float weak_thresh = 30.0f, float strong_thresh = 60.0f,
			Mat mask = Mat());
		/*
		加载模型
		...
		*/
		...
		vector<Match> Matching1(Mat source, float score_thresh = 0.9f, float overlap = 0.4f,
			float mag_thresh = 30.f, float greediness = 0.8f, PyramidLevel pyrd_level = PyramidLevel_3,
			int T = 2, int top_k = 0, MatchingStrategy strategy = Strategy_Accurate, const Mat mask = Mat());
		void DrawMatches(Mat& image, vector<Match> matches, Scalar color);
		cv::Mat DrawMatches1(cv::Mat& image, std::vector<Match>& matches, cv::Scalar color);
		vector<Point2f>ReadAndRotateEdgePoints(const string& filename, Size imageSize, float angle_deg1, float scale1 );
		void saveMatchesToXml(const std::string& filePath, const std::vector<Match>& matches);
	...
	protected:
		...
		vector<Match> MatchingPyrd180(Mat src, PyramidLevel pl,  vector<Match> matches,vector<int> region_idxes = vector<int>());
		...
		//旋转0度特征点
		int addTemplate_rotate(int p, double theta);
